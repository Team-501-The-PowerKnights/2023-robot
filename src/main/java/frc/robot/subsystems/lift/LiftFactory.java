...
public class LiftFactory {
   ...
   /** Our classes' logger **/
   private static final Logger logger = PKLogger.getLogger(LiftFactory.class.getName());
   ...
   public static synchronized void constructInstance() {
      SmartDashboard.putNumber(TelemetryNames.Lift.status, PKStatus.inProgress.tlmValue);
   ...
   private static void loadImplementationClass(String myClassName) {
      String myPkgName = LiftFactory.class.getPackage().getName();
      ...
      try {
         ...
         ourInstance = (ILiftSubsystem) myObject;
         SmartDashboard.putNumber(TelemetryNames.Lift.status, PKStatus.success.tlmValue);
      } catch (ClassNotFoundException | InstantiationException | IllegalAccessException ex) {
         logger.error("failed to load class; instantiating default stub for: {}", myName);
         ProblemTracker.addError();
         ourInstance = new StubLiftSubsystem();
         ourInstance.setDefaultCommand(new LiftDoNothing());
         SmartDashboard.putNumber(TelemetryNames.Lift.status, PKStatus.degraded.tlmValue);
      }
      SmartDashboard.putString(TelemetryNames.Lift.implClass, ourInstance.getClass().getSimpleName());
   }
